...
# It is easier to overwrite this function here than of doing sed.
def upload_public_dir(files, message):
    for dirpath, dirnames, filenames in os.walk(os.getcwd()):
        print("dirpath=%s dirnames=%s filenames=%s\n" %(dirpath, dirnames, filenames))
        relpath=os.path.relpath(dirpath, os.getcwd())
        destDir=os.path.join(FEED_UPLOAD_DIR, relpath)
        try:
            os.makedirs(destDir)
        except OSError:
